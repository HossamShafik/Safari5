...
class TourRatingStream extends StatelessWidget {
 final Tour tourId;
  ...
  Widget build(BuildContext context) {
    return  StreamProvider<List<TourRating>>.value(
      value: DataBase().getAllTourComment(tourId),
      catchError: (_, err) => throw Exception(err),
      child: TourRatingWidget(),
    );
  }
}
...
class TourRatingWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final tourRatingList = Provider.of<List<TourRating>>(context);

    return Expanded(
      child: ListView.builder(
        itemCount: tourRatingList != null && tourRatingList.length > 0 ? tourRatingList.length : 0,
        itemBuilder: (context, index) {
          final TourRating currentRate = tourRatingList[index];
